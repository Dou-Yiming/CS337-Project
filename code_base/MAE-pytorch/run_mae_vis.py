...
def get_args():
    parser = argparse.ArgumentParser(
        'MAE visualization reconstruction script', add_help=False)
...
def main(args):
    ...
    with torch.no_grad():
        ...
        img_squeeze = rearrange(
            ori_img, 'b c (h p1) (w p2) -> b (h w) (p1 p2) c', p1=patch_size[0], p2=patch_size[0])
        ...
        rec_img = rec_img * (img_squeeze.var(dim=-2, unbiased=True,
                                             keepdim=True).sqrt() + 1e-9) + img_squeeze.mean(dim=-2, keepdim=True)
